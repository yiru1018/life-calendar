...
import SwitchBtn from '../event_modal/SwitchBtn';
import UpdateStartCalendar from './UpdateStartCalendar';
import UpdateEndCalendar from './UpdateEndCalendar';
...
function UpdateSetTime({
  ...
  const showDay = (slcDay) => {
    ...
    const result = `${dayjs(slcDay).format('M[月]D[日]')}(星期${
      week[dayjs(slcDay).format('d')]
    })`;
    return result;
  };
